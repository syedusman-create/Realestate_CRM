    ...
    return (
      <Card>
        <CardContent className="py-12 text-center">
          ...
          <Link href="/dashboard/inventory/new" className="mt-4 inline-flex h-9 items-center rounded-md bg-primary px-4 text-sm font-medium text-primary-foreground hover:opacity-90">
            Add property
          </Link>
        </CardContent>
      </Card>
          ...
          <table className="w-full min-w-[1250px] text-sm">
            <thead className="border-b bg-muted/40">
              <tr className="text-left text-muted-foreground">
                ...
                <th className="px-4 py-3 font-medium">Phase / Tower</th>
            ...
            <tbody className="divide-y">
              {items.map((item) => (
                <tr key={item.id} className="transition-colors hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3">
                    {item.project?.name ?? '—'}
                    {item.project?.city ? <div className="text-xs text-muted-foreground">{item.project.city}</div> : null}
                  </td>
                  <td className="px-4 py-3">
                    <div>{item.tower?.name ?? 'No tower'}</div>
                    <div className="text-xs text-muted-foreground">{item.phase?.name ?? 'No phase'}</div>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    {item.listing ? (
                      <div>
                        <div className="font-medium">{formatListingType(item.listing.listing_type)}</div>
                        <div className="text-xs text-muted-foreground">{item.listing.status}</div>
                      </div>
