...
export async function saveDeal(_: DealActionState = initialState, formData: FormData): Promise<DealActionState> {
  try {
    const { supabase, userId } = await context(); const db = supabase as any
    const dealId = text(formData.get('deal_id')); const leadId = text(formData.get('lead_id')); const stageId = text(formData.get('stage_id')) || null
    const dealName = text(formData.get('deal_name')); const dealValue = money(text(formData.get('deal_value'))); const expectedClose = text(formData.get('expected_close_date')) || null
    const probabilityText = text(formData.get('probability')); const probability = probabilityText ? Number(probabilityText) : null
    const status = text(formData.get('status')) || 'open'; const lostReason = text(formData.get('lost_reason')) || null; const notes = text(formData.get('notes')) || null
    ...
    const { data: existing, error: existingError } = dealId ? await db.from('deals').select('id, lead_id, owner_user_id, pipeline_id').eq('id', dealId).maybeSingle() : await db.from('deals').select('id, lead_id, owner_user_id, pipeline_id').eq('lead_id', leadId).maybeSingle()
    ...
    if (existing.owner_user_id && existing.owner_user_id !== userId) { const { data: manager } = await supabase.rpc('crm_can_manage_tenant'); if (!manager) return { ok: false, message: 'You can only edit opportunities assigned to you.' } }
    if (stageId) { const { data: stage, error: stageError } = await supabase.from('pipeline_stages').select('id, pipeline_id').eq('id', stageId).maybeSingle(); if (stageError || !stage) return { ok: false, message: 'Selected stage is invalid.' }; if (existing.pipeline_id && stage.pipeline_id !== existing.pipeline_id) return { ok: false, message: 'Stage does not belong to this pipeline.' } }
...
export async function createDeal(_: DealActionState = initialState, formData: FormData): Promise<DealActionState> {
  try {
    const { supabase, userId } = await context(); const leadId = text(formData.get('lead_id')); const dealName = text(formData.get('deal_name'))
...
export async function saveDealForm(formData: FormData): Promise<void> { await saveDeal(initialState, formData) }
export async function createDealForm(formData: FormData): Promise<void> { await createDeal(initialState, formData) }
