...
export default async function DealsPage({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  ...
  return <main className="page">
    ...
    <nav className="tabs">{statusTabs.map((tab) => <Link key={tab} className={status === tab ? 'tab active' : 'tab'} href={tab === 'all' ? '/dashboard/deals' : `/dashboard/deals?status=${tab}`}>{`${tab.charAt(0).toUpperCase()}${tab.slice(1)}`}</Link>)}</nav>
...
function formatMoney(value: number): string { if (!value) return '₹0'; if (value >= 10000000) return `₹${(value / 10000000).toFixed(2)} Cr`; if (value >= 100000) return `₹${(value / 100000).toFixed(1)} L`; return `₹${value.toLocaleString('en-IN')}` }
