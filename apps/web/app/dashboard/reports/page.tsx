...
export default async function ReportsPage() {
  ...
  const [{ data: metrics }, { data: agents }, { data: stages }, { data: sources }, { data: deals }] = await Promise.all([
    ...
    db.from('deal_pipeline').select('deal_id, lead_id, deal_name, person_name, deal_value, expected_close_date, probability, stage_probability, status, stage_name, owner_name').eq('status', 'open').order('expected_close_date', { ascending: true }).limit(12),
  ])
